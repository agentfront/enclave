...
EnclaveJS is a streaming runtime layer built on top of `enclave-vm` that enables real-time code execution with tool orchestration, session management, and client SDKs for browser and React applications.
...
│  (@enclavejs/   │ HTTP │  (@enclavejs/   │  WS  │  (@enclavejs/   │
...
<CardGroup cols={2}>
  <Card title="@enclavejs/types" icon="code" href="/enclavejs/types">
    Protocol types and Zod schemas for the streaming runtime
  </Card>
  <Card title="@enclavejs/stream" icon="wave-pulse" href="/enclavejs/stream">
  ...
  <Card title="@enclavejs/runtime" icon="microchip" href="/enclavejs/runtime">
...
```typescript
import { Broker, createBroker } from '@enclavejs/broker';
import { z } from 'zod';
...
- [npm @enclavejs](https://www.npmjs.com/org/enclavejs)
