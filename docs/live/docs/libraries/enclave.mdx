...
- **Memory-aware Limits**: Allocation tracking with automatic `MemoryLimitError` handling
...
## AST Validation Presets

Choose the level of AST validation enforced before any script executes by passing the `preset` option when constructing an `Enclave`. The default `agentscript` preset mirrors the AgentScript runtime, while the other presets trade flexibility for additional hardening.

| Preset | When to use it |
| ------ | -------------- |
| `agentscript` (default) | AI agent tool orchestration with AgentScript helpers |
| `strict` | Maximum restrictions for highly regulated workloads |
| `secure` | High security with enough ergonomics for most production agents |
| `standard` | Balanced validation for mixed workloads |
| `permissive` | Trusted internal scripts that only need sandboxing for isolation |
...
All object-valued globals are automatically wrapped in secure proxies, so even permissive presets retain prototype-chain protections.
...
Setting the `memoryLimit` option now instruments string and array allocations so you can surface peak usage and fail fast when untrusted code attempts to exhaust memory.

- When a limit is configured, `result.stats.memoryUsage` reports the peak tracked bytes for each execution.
- If the limit is exceeded the sandbox returns `success: false` with an error that includes the `MEMORY_LIMIT_EXCEEDED` code plus `{ usedBytes, limitBytes }` data.
- `MemoryTracker` and `MemoryLimitError` are exported if you want to perform additional manual tracking (for example when writing a custom adapter).
...
```typescript
import { MemoryTracker } from 'enclave-vm';
...
### Throwing on Blocked Properties

Security levels `STRICT`, `SECURE`, and `STANDARD` now throw a `SecurityError` whenever blocked properties such as `constructor`, `__proto__`, or `prototype` are touched. This makes sandbox escapes explicit instead of silently returning `undefined`. Set `throwOnBlocked: false` if you need the legacy behavior.

```typescript
const enclave = new Enclave({
  securityLevel: 'SECURE',
  secureProxyConfig: {
    throwOnBlocked: false, // optional override
  },
});
```
```

> **Note:** `clearProxyCache()` is intentionally unsupported and now throws an error. The proxy cache relies on garbage collection; manual clearing is unnecessary and prevented to avoid weakening the isolation guarantees.
