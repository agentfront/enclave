...
- `modelId` (required): Hugging Face model identifier such as `Xenova/codebert-base`
- `mode` (optional): `'classification'` (default) or `'similarity'`
- `cacheDir` (optional): Override where models are stored on disk — defaults to `~/.enclave/models`
- `vectoriaConfig` (optional): Configure VectoriaDB when `mode` is `'similarity'`

## Links

- [GitHub](https://github.com/agentfront/enclave/tree/main/libs/enclave-vm)
